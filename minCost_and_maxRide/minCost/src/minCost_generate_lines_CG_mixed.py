...
import minCostLP
import minCostLPRelaxed

import instance

import minCost_generate_lines_CG_exact 
import minCost_generate_lines_CG_relaxed 

from paths import output_path, save_pickle, data_path, output_dir, load_pickle
...
def main(city, which_method, gamma, cap, unit_dist, num_rounds, perc_bus_edges_selected, flex_dist, mip_gap, time_limit, num_sol, max_travel, saved_folder,\
		 detour_coeff, demand_perc, include_naive, len_naive, cost_factor_naive):
	...
	lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict = minCostLP.LPPrep(lines_mixed_exact, node_list, ori_nodes, bus_nodes, od_pairs, dict_stlInd_uInd_mixed_exact)


	# run LP only once
	m, obj, C4_toRemove, C5_toRemove, C6_toFix, f_LP, y_LP, w_LP, z_LP, q_LP, x_LP, Y_LP = minCostLP.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_mixed_exact, lines_edge, cost_edges, cost_lines_mixed_exact, demand_dic, cap_lines_mixed_exact,\
	...
	lines_edge_rela, list_f_sluv_rela, list_y_slv_rela, list_w_slv_rela, list_z_stv_rela, list_z_rela, st_dict_rela, ls_dict_rela, svl_dict_rela, z_uvs_dict_rela, y_uvl_dict_rela, all_lines_edge, dict_edge_lines\
			= minCostLPRelaxed.LPPrep(lines_mixed_relaxed, node_list, ori_nodes, bus_nodes, od_pairs, dict_stlInd_uInd_mixed_relaxed)


	# run LPRelaxed only once
	m_rela, obj_rela, C4_toRemove_rela, C5_toRemove_rela, C6_toRemove_rela, f_rela, y_rela, w_rela, z_rela, q_rela, x_rela, Y_rela =\
			minCostLPRelaxed.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_mixed_relaxed, lines_edge_rela, cost_edges, cost_lines_mixed_relaxed, demand_dic, cap_lines_mixed_relaxed,\
			list_f_sluv_rela, list_y_slv_rela, list_w_slv_rela, list_z_stv_rela, list_z_rela, bus_nodes, st_dict_rela, ls_dict_rela, svl_dict_rela, z_uvs_dict_rela, y_uvl_dict_rela, all_lines_edge, dict_edge_lines, demand_perc)
	
	m_rela.write(output_path(city, saved_folder, 'LP_rela.mps'))
	m_rela.write(output_path(city, saved_folder, 'LP_rela.lp'))
 
 
	r_current = minCostLPRelaxed.getShadowPrice(m_rela, "r_relaxed")
	...
	bus_nodes_selected_x_relaxed, bus_edges_selected_x_relaxed = minCost_generate_lines_CG_relaxed.subPrep(bus_nodes_selected_relaxed, bus_edges_selected_relaxed)

	# run RELAXED subproblem 
	log_directory = output_path(city, saved_folder, 'minCost_generate_lines_CG_relaxed_log.txt')
	mSubprob_relaxed, subObj_relaxed, x_relaxed, numVars_relaxed = minCost_generate_lines_CG_relaxed.subproblem(r_current, bus_nodes_selected_relaxed, cost_edges, cap, gamma, bus_edges_selected_relaxed, bus_nodes_selected_x_relaxed, bus_edges_selected_x_relaxed, mip_gap, time_limit, num_sol, max_travel, detour_coeff, log_directory)
	...
	bus_edges_selected_exact, bus_nodes_selected_exact =  minCost_generate_lines_CG_exact.trim(k_current, p_current, cap, gamma, bus_nodes, bus_nodes_x_exact, ori_nodes, cost_edges, bus_edges, bus_edges_x_exact,\
	...
	bus_nodes_selected_x_exact, bus_edges_selected_x_exact, dict_bus_to_selected, dict_bus_from_selected,\
	 dict_bus_to_selected_x, dict_bus_from_selected_x, sBs_list_selected, list_p_sv_selected\
		  = minCost_generate_lines_CG_exact.subPrep(bus_nodes_selected_exact, bus_edges_selected_exact, ori_nodes, flex_dist, cost_edges, od_pairs)
	
	
	# run subproblem based on the trimmed graph
	log_directory = output_path(city, saved_folder, 'minCost_generate_lines_CG_exact_log.txt')
	mSubprob_exact, subObj_exact, h_exact, numVars_exact = minCost_generate_lines_CG_exact.subproblem(k_current, p_current, cap, gamma, bus_nodes_selected_exact, bus_nodes_selected_x_exact, ori_nodes, cost_edges, bus_edges_selected_exact, bus_edges_selected_x_exact,\
	...
	counter_outer = 0 # count total number of subproblem iteration, either RELAXED or EXACT
		...
		if counter_outer < num_rounds and subObj_relaxed < 0 and len(bus_edges_selected_relaxed) > 0:
			...
			for i in range(num_sol):
				if nSolutions > i:
					...
					tour, cycles_list, irrelevant_edges = minCost_generate_lines_CG_relaxed.oneSol(x_edge)
	 
					if tour not in lines_mixed_relaxed and len(cycles_list) <= 0 and len(irrelevant_edges) <= 0:
						...
						lines_mixed_relaxed, cost_lines_mixed_relaxed, cap_lines_mixed_relaxed, pair_lineInd_dict_mixed_relaxed, dict_stlInd_uInd_mixed_relaxed =\
							minCostLPRelaxed.newLineVars(tour, lines_mixed_relaxed, cost_lines_mixed_relaxed, cap_lines_mixed_relaxed, pair_lineInd_dict_mixed_relaxed, dict_stlInd_uInd_mixed_relaxed, gamma, cap, cost_edges, node_list, flex_dist, od_pairs, unit_dist)
		
					
						# re-generate LPRelaxed (add additional vars and constraints)
						m_rela, C4_toRemove_rela, C5_toRemove_rela, C6_toRemove_rela, f_rela, y_rela, w_rela, z_rela, q_rela, x_rela, Y_rela, lines_edge_rela, list_f_sluv_rela, list_y_slv_rela, list_w_slv_rela, list_z_stv_rela, list_z_rela\
						, st_dict_rela, ls_dict_rela, svl_dict_rela, z_uvs_dict_rela, y_uvl_dict_rela \
							= minCostLPRelaxed.newLineLP(m_rela, C4_toRemove_rela, C5_toRemove_rela, C6_toRemove_rela, cap, od_pairs, node_list, lines_mixed_relaxed,\
						...
						, bus_nodes, st_dict_rela, ls_dict_rela, svl_dict_rela, z_uvs_dict_rela, y_uvl_dict_rela, f_rela, y_rela, w_rela, z_rela, q_rela, x_rela, Y_rela)
						...
						m, C4_toRemove, C5_toRemove, C6_toFix, f_LP, y_LP, w_LP, z_LP, q_LP, x_LP, Y_LP, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z\
		  				, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict = minCostLP.newLineLP(m, C4_toRemove, C5_toRemove, C6_toFix, cap, od_pairs, node_list, lines_mixed_exact, lines_edge, ori_nodes, cost_edges, tour, cost_lines_mixed_exact, cap_lines_mixed_exact, dict_stlInd_uInd_mixed_exact, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z\
			...
			if len(bus_edges_selected_relaxed) > 0:
				# for subproblem
				bus_nodes_selected_x_relaxed, bus_edges_selected_x_relaxed = minCost_generate_lines_CG_relaxed.subPrep(bus_nodes_selected_relaxed, bus_edges_selected_relaxed)
		...
		if counter_outer < num_rounds and subObj_exact < 0 and len(bus_edges_selected_exact) > 0:
			...
			for i in range(num_sol):
				if (nSolutions > i):
					...
					tour, cycles_list, irrelevant_edges = minCost_generate_lines_CG_exact.oneSol(h_edge)
	 
					if tour not in lines_mixed_exact and len(cycles_list) <= 0 and len(irrelevant_edges) <= 0:	
						# create variables needed for running LP after generating the new line
						lines_mixed_exact, cost_lines_mixed_exact, cap_lines_mixed_exact, pair_lineInd_dict_mixed_exact, dict_stlInd_uInd_mixed_exact = minCostLP.newLineVars(tour, lines_mixed_exact, cost_lines_mixed_exact, cap_lines_mixed_exact, pair_lineInd_dict_mixed_exact, dict_stlInd_uInd_mixed_exact, gamma, cap, cost_edges, node_list, flex_dist, od_pairs, unit_dist)
			...
			if len(bus_edges_selected_exact) > 0:
				...
				# run subproblem based on the trimmed graph
				log_directory = output_path(city, saved_folder, 'minCost_generate_lines_CG_exact_log.txt')
				mSubprob_exact, subObj_exact, h_exact, numVars_exact = minCost_generate_lines_CG_exact.subproblem(k_current, p_current, cap, gamma, bus_nodes_selected_exact, bus_nodes_selected_x_exact, ori_nodes, cost_edges, bus_edges_selected_exact, bus_edges_selected_x_exact, sBs_list_selected,\
