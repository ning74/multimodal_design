...
def subproblem(r_current, node_list, cost_edges, cap, gamma, edge_list, node_list_x, edge_list_x, mip_gap, time_limit, num_sol, max_travel, detour_coeff, log_directory):
	mSubprob = gp.Model('modelSubprob')
	print('run subproblem relaxed~')
	...
	for item in edge_list:
		obj_func += (gamma*cost_edges[item]-cap*r_current[item] + gamma*cost_edges[item[1],item[0]]-cap*r_current[item[1],item[0]]) * h[item]
...
def subproblemLP(r_current, node_list, cost_edges, cap, gamma, edge_list, node_list_x, edge_list_x, max_travel):
...
def trim(r_current, node_list, cost_edges, cap, gamma, edge_list, node_list_x, edge_list_x, max_travel, num_bus_edges_selected):
	bus_edges_selected = set()
	bus_nodes_selected = set()
 
	start_time = time.time()
	
	mSubprobLP, subObjLP, x_edge = subproblemLP(r_current, node_list, cost_edges, cap, gamma, edge_list, node_list_x, edge_list_x, max_travel)
...
def main(city, which_method, gamma, cap, unit_dist, num_rounds, perc_bus_edges_selected, flex_dist, mip_gap, time_limit, num_sol, max_travel, saved_folder, detour_coeff, demand_perc, include_naive, len_naive, cost_factor_naive):
	...
	# run LP only once
	m, obj, C4_toRemove, C5_toRemove, C6_toFix, f_LP, y_LP, w_LP, z_LP, q_LP, x_LP, Y_LP = minCostLP.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_current, lines_edge, cost_edges, cost_lines_current, demand_dic, cap_lines_current,\
	...
	lines_edge_rela, list_f_sluv_rela, list_y_slv_rela, list_w_slv_rela, list_z_stv_rela, list_z_rela, st_dict_rela, ls_dict_rela, svl_dict_rela, z_uvs_dict_rela, y_uvl_dict_rela, all_lines_edge, dict_edge_lines\
			= minCostLPRelaxed.LPPrep(lines_current_rela, node_list, ori_nodes, bus_nodes, od_pairs, dict_stlInd_uInd_current_rela)
	...
	m_rela, obj_rela, C4_toRemove_rela, C5_toRemove_rela, C6_toRemove_rela, f_rela, y_rela, w_rela, z_rela, q_rela, x_rela, Y_rela =\
			minCostLPRelaxed.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_current_rela, lines_edge_rela, cost_edges, cost_lines_current_rela, demand_dic, cap_lines_current_rela,\
	...
	bus_edges_selected, bus_nodes_selected = trim(r_current, bus_nodes, cost_edges, cap, gamma, bus_edges, bus_nodes_x, bus_edges_x, max_travel, num_bus_edges_selected)
	...
	mSubprob, subObj, x, numVars = subproblem(r_current, bus_nodes_selected, cost_edges, cap, gamma, bus_edges_selected, bus_nodes_selected_x, bus_edges_selected_x, mip_gap, time_limit, num_sol, max_travel, detour_coeff, log_directory)
		...
		for i in range(num_sol):
			if nSolutions > i:
				...
				lines_current, cost_lines_current, cap_lines_current, pair_lineInd_dict_current, dict_stlInd_uInd_current =\
			  		minCostLP.newLineVars(tour, lines_current, cost_lines_current, cap_lines_current, pair_lineInd_dict_current, dict_stlInd_uInd_current, gamma, cap, cost_edges, node_list, flex_dist, od_pairs, unit_dist)
				...
				lines_current_rela, cost_lines_current_rela, cap_lines_current_rela, pair_lineInd_dict_current_rela, dict_stlInd_uInd_current_rela =\
			  		minCostLPRelaxed.newLineVars(tour, lines_current_rela, cost_lines_current_rela, cap_lines_current_rela, pair_lineInd_dict_current_rela, dict_stlInd_uInd_current_rela, gamma, cap, cost_edges, node_list, flex_dist, od_pairs, unit_dist)
				...
				m_rela, C4_toRemove_rela, C5_toRemove_rela, C6_toRemove_rela, f_rela, y_rela, w_rela, z_rela, q_rela, x_rela, Y_rela, lines_edge_rela, list_f_sluv_rela, list_y_slv_rela, list_w_slv_rela, list_z_stv_rela, list_z_rela\
		  		, st_dict_rela, ls_dict_rela, svl_dict_rela, z_uvs_dict_rela, y_uvl_dict_rela \
					= minCostLPRelaxed.newLineLP(m_rela, C4_toRemove_rela, C5_toRemove_rela, C6_toRemove_rela, cap, od_pairs, node_list, lines_current_rela,\
