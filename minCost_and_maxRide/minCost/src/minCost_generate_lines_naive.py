...
import instance
import minCostLP
from paths import output_path, save_pickle, output_dir
...
def main(city, which_method, gamma, cap, unit_dist, flex_dist, saved_folder, demand_perc):
	...
	while len(bus_edges_temp) > 2:
		...
		# add the first edge into new_path
		new_path.append(first_edge[0])
		new_path.append(first_edge[1])

		# remove first edge and its reverse edge from bus_edges_temp
