...
def main(city, gamma, cap, flex_dist, mip_gap, unit_dist, top, bottom, saved_folder, firstSelect, secondSelectStep, num_finalLines, saved_folder_new, mip_focus, time_limit, demand_perc):     
	...
	pair_lineInd_dict_current, dict_stlInd_uInd_current, cost_lines_current, cap_lines_current = minCostLP.extract_lines(city, demand_dic, gamma, cap, flex_dist, unit_dist, lines_current)
	...
	# run once 
	m_int, obj_int, C4_toRemove_int, C5_toRemove_int, C6_toFix_int, f_int, y_int, w_int, z_int, q_int, x_int, Y_int = minCostLP.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_current, lines_edge, cost_edges, cost_lines_current, demand_dic, cap_lines_current,\
	...
	path_name = result_path(city, 'heur_optSol_taxiOnly[city@{}|gamma@{}|cap@{}|mip_gap@{}|top@{}|bottom@{}|saved_folder@{}].txt'\
		.format(city, gamma, cap, mip_gap, top, bottom, saved_folder))
	...
	# run LP only once
	m_LP, obj_LP, C4_toRemove_LP, C5_toRemove_LP, C6_toFix_LP, f_LP, y_LP, w_LP, z_LP, q_LP, x_LP, Y_LP = minCostLP.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_current, lines_edge, cost_edges, cost_lines_current, demand_dic, cap_lines_current,\
		  list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, demand_perc)
	


	
	m_LP.write(heur_path(city, saved_folder_new, 'LP_gradualSelect_fullModel.mps'))
	m_LP.write(heur_path(city, saved_folder_new, 'LP_gradualSelect_fullModel.lp'))
 
 
	path_name = heur_path(city, saved_folder_new, 'heur_gradualSelect_LPsol_fullModel.txt')
	with open(path_name, "w") as file:
	...
	Y_RC_normalized_ordered.reverse()
	...
	with open(heur_path(city, saved_folder_new, 'heur_gradualSelect_selectedLineIndex_numGradualSelected@{}_from{}.pkl'.format(firstSelect, num_originalLines)), 'wb') as file:
	...
	m_LP.write(heur_path(city, saved_folder_new, 'LP_gradualSelect_{}lines.mps'.format(firstSelect)))
	m_LP.write(heur_path(city, saved_folder_new, 'LP_gradualSelect_{}lines.lp'.format(firstSelect)))
 
 
	path_name = heur_path(city, saved_folder_new, 'heur_gradualSelect_LPsol_firstSelect@{}.txt'.format(firstSelect))
	with open(path_name, "w") as file:
		file.write(f"Objective Value = {m_LP.ObjVal}\n")
		for v in m_LP.getVars():
			# Write each line to the file
	...
	with open(heur_path(city, saved_folder_new, 'heur_gradualSelect_selectedLineIndex_numGradualSelected@{}_from{}.pkl'.format(numSelect_real, firstSelect)), 'wb') as file:
		pickle.dump(line_select, file)
	...
	while numSelect_real > num_finalLines:
		...
		path_name = heur_path(city, saved_folder_new, 'heur_gradualSelect_LPsol_nonzeroLines@{}_from{}.txt'.format(numSelect_real + secondSelectStep, firstSelect))
		with open(path_name, "w") as file:
			file.write(f"Objective Value = {m_LP.ObjVal}\n")
			for v in m_LP.getVars():
				# Write each line to the file
	...
	with open(heur_path(city, saved_folder_new, 'heur_gradualSelect_selectedLineIndex_numGradualSelected@{}_from{}.pkl'.format(firstSelect, num_originalLines), create=False), 'rb') as file:
		lines_firstSelect_index = pickle.load(file)
 
	with open(heur_path(city, saved_folder_new, 'heur_gradualSelect_selectedLineIndex_numGradualSelected@{}_from{}.pkl'.format(num_finalLines, firstSelect), create=False), 'rb') as file:
	...
	log_name = result_path(city, 'heur_log[city@{}|gamma@{}|cap@{}|mip_gap@{}|top@{}|bottom@{}|firstSelect@{}|secondSelectStep@{}|num_finalLines@{}|saved_folder@{}].txt'\
		.format(city, gamma, cap, mip_gap, top, bottom, firstSelect, secondSelectStep, num_finalLines, saved_folder))
	
	# run once
	m_MIP, obj_MIP, C4_toRemove_MIP, C5_toRemove_MIP, f_MIP, y_MIP, w_MIP, z_MIP, q_MIP, x_MIP, Y_MIP = minCostMIP.getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines_current, lines_edge, cost_edges, cost_lines_current, demand_dic, cap_lines_current,\
			list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, mip_gap, log_name, cap, mip_focus, time_limit, demand_perc)
		

 
 
	# write gurobi optimal solutions
	path_name = result_path(city, 'heur_optSol[city@{}|gamma@{}|cap@{}|mip_gap@{}|top@{}|bottom@{}|firstSelect@{}|secondSelectStep@{}|num_finalLines@{}|saved_folder@{}].txt'\
		.format(city, gamma, cap, mip_gap, top, bottom, firstSelect, secondSelectStep, num_finalLines, saved_folder))
...
if __name__ == "__main__":
	...
	parser.add_argument('--bottom', required=True) # 800
	...
	parser.add_argument('--saved_folder_new', required=True) # multimodal_budget_MIP
	...
	# numGenerateRound = int(args.numGenerateRound)
	saved_folder_new = args.saved_folder_new
	mip_focus = int(args.mip_focus)
	time_limit = int(args.time_limit)
	demand_perc = float(args.demand_perc)
 
	start = time.time()
	main(city=city, gamma=gamma, cap=cap, flex_dist=flex_dist, mip_gap=mip_gap, unit_dist=unit_dist, top=top, bottom=bottom, saved_folder=saved_folder,\
	 firstSelect=firstSelect, secondSelectStep=secondSelectStep, num_finalLines=num_finalLines, saved_folder_new=saved_folder_new, mip_focus=mip_focus, time_limit=time_limit, demand_perc=demand_perc)
