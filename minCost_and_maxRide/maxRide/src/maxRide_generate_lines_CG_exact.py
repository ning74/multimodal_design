...
def subproblem(k_current, p_current, bud_current, cap, gamma, node_list, node_list_x, ori_nodes, cost_edges, edge_list, edge_list_x,\
	...
	obj_func = gp.quicksum(x[item]*(-k_current[item]) for item in sBs_list) + gp.quicksum(y[item]*p_current[item] for item in list_p_sv)\
		  - gp.quicksum((bud_current * (gamma / cap) * (cost_edges[item]+cost_edges[item[1],item[0]]) * h[item]) for item in edge_list)
...
def subproblemLP(k_current, p_current, bud_current, cap, gamma, node_list, node_list_x, ori_nodes, cost_edges, edge_list, edge_list_x,\
...
def trim(k_current, p_current, bud_current, cap, gamma, bus_nodes, bus_nodes_x, ori_nodes, cost_edges, bus_edges, bus_edges_x,\
	...
	mSubprobLP, subObjLP, h_edge = subproblemLP(k_current, p_current, bud_current, cap, gamma, bus_nodes, bus_nodes_x, ori_nodes, cost_edges, bus_edges, bus_edges_x,\
							   sBs_list, list_p_sv, dict_bus_to, dict_bus_from, dict_bus_to_x, dict_bus_from_x, max_travel)
...
def main(city, which_method, which_trim, gamma, cap, budget, unit_dist, num_rounds, perc_bus_edges_selected, flex_dist, mip_gap, time_limit, num_sol, max_travel, saved_folder,\
	...
	bud_current = maxRideLP.getShadowPrice(m, "bud")[0]
	...
	mSubprob, subObj, h, numVars = subproblem(k_current, p_current, bud_current, cap, gamma, bus_nodes_selected, bus_nodes_selected_x, ori_nodes, cost_edges, bus_edges_selected, bus_edges_selected_x,\
	...
	while continue_flag == True and counter_outer < num_rounds and subObj > 0:
		...
		for i in range(num_sol):
			if (nSolutions > i):
				...
				tour, cycles_list, irrevevant_edges = oneSol(h_edge)
		...
		if len(bus_edges_selected) == 0:
			...
			# run subprblem based on the trimmed graph
			log_directory = output_path(city, saved_folder, 'generate_lines_CG_exact_log.txt')
			mSubprob, subObj, h, numVars = subproblem(k_current, p_current, bud_current, cap, gamma, bus_nodes_selected, bus_nodes_selected_x, ori_nodes, cost_edges, bus_edges_selected, bus_edges_selected_x, sBs_list_selected,\
