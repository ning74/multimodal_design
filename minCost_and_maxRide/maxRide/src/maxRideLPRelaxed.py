...
def createVars(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, transit):
...
def createConstrs(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, demand_dic, cap_lines, budget, cost_edges, cost_lines, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z,\
				  f, y, w, z, q, x, Y, bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, all_lines_edge, dict_edge_lines, alpha):
	...
	C6_toRemove = m.addConstrs((-gp.quicksum(cap_lines[l]*Y[l] for l in dict_edge_lines[edge]) + \
					 gp.quicksum(f[s,l,u,v] for (s,l,u,v) in list_f_sluv if u == edge[0] and v == edge[1]) <= 0 for edge in all_lines_edge), name = 'r_relaxed')
	
	
	
	m.addConstrs((gp.quicksum(x[v,uu] for v in node_list if v != uu) - gp.quicksum(x[uu,v] for v in node_list if v != uu) == 0 for uu in node_list), name = 'constraint_7')
	...
	return C4_toRemove, C5_toRemove, C6_toRemove, C8_toRemove
...
def getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, cost_edges, cost_lines, demand_dic, cap_lines, budget, list_f_sluv,\
	       list_y_slv, list_w_slv, list_z_stv, list_z, bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, all_lines_edge, dict_edge_lines, alpha, transit):
	...
	# create variables
	f, y, w, z, q, x, Y = createVars(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, transit)
	...
	# create constraints 
	C4_toRemove, C5_toRemove, C6_toRemove, C8_toRemove = createConstrs(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, demand_dic, cap_lines, budget, cost_edges, cost_lines, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z,\
				  f, y, w, z, q, x, Y, bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, all_lines_edge, dict_edge_lines, alpha)
	...
	return m, m.ObjVal, C4_toRemove, C5_toRemove, C6_toRemove, C8_toRemove, f, y, w, z, q, x, Y
...
def LPPrep(lines, node_list, ori_nodes, bus_nodes, od_pairs, dict_stlInd_uInd):
	...
	all_lines_edge = set()
	for l in range(len(lines)):
		for edge in lines_edge[l]:
			all_lines_edge.add(edge)
	all_lines_edge = list(all_lines_edge)

	dict_edge_lines = {}
	for edge in all_lines_edge:
		e_lines = set()
		for l in range(len(lines_edge)):
			if edge in lines_edge[l]:
				e_lines.add(l)
		dict_edge_lines[edge] = e_lines


	return lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, all_lines_edge, dict_edge_lines


def newLineLP(m, C4_toRemove, C5_toRemove, C6_toRemove, C8_toRemove, cap, budget, od_pairs, node_list, new_lines, lines_edge, ori_nodes, cost_edges, new_line, new_cost_lines, new_cap_lines, new_dict_stlInd_uInd, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z\
	      , bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, f, y, w, z, q, x, Y, alpha):
	...
	Y.update({ll: m.addVar(lb = 0, ub = float('inf'), vtype = GRB.CONTINUOUS, name = 'var_Y[{}]'.format(ll))})
	...
	m.remove(C6_toRemove)
	C6_toRemove = m.addConstrs((-gp.quicksum(new_cap_lines[l]*Y[l] for l in dict_edge_lines[edge]) + \
					 gp.quicksum(f[s,l,u,v] for (s,l,u,v) in list_f_sluv if u == edge[0] and v == edge[1]) <= 0 for edge in all_lines_edge), name = 'r_relaxed')
	
	m.remove(C8_toRemove)
	C8_toRemove = m.addConstr(gp.quicksum(x[u,v]*cost_edges[(u,v)]*alpha for u in node_list for v in node_list if v != u)\
		+ gp.quicksum(new_cost_lines[l]*Y[l] for l in range(len(new_lines))) <= budget, name = 'bud')



	return m, C4_toRemove, C5_toRemove, C6_toRemove, C8_toRemove, f, y, w, z, q, x, Y, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z\
...
def newLineVars(new_line, lines, cost_lines, cap_lines, pair_lineInd_dict, dict_stlInd_uInd, gamma, cap, cost_edges, node_list, flex_dist, od_pairs, unit_dist):
	...
	pair_lineInd_dict_temp = {k:v for (k,v) in new_pair_lineInd_dict.items() if len(v) > 0}
	...
	if m.status == gp.GRB.OPTIMAL:
		for constr in m.getConstrs():
			if constr.ConstrName.split('[')[0] == consrtName_string:
				if consrtName_string == "a" or consrtName_string == "k" or consrtName_string == "p" or consrtName_string == "r_relaxed": 
