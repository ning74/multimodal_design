...
def createVars(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, bus_nodes, transit):
...
def createConstrs(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, demand_dic, cap_lines, budget, cost_edges, cost_lines, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z,\
	...
	constraint_time_1 = time.process_time() - constraint_time 

	m.addConstrs((gp.quicksum(w[s,l,v] for (s,l,v) in [(ss,ll,vv)] if (s,l,v) in list_w_slv) + \
					gp.quicksum(f[ss,ll,vv,edge[1]] for edge in lines_edge[ll] if edge[0] == vv) - \
					gp.quicksum(y[s,l,v] for (s,l,v) in [(ss,ll,vv)] if (s,l,v) in list_y_slv) - \
					gp.quicksum(f[ss,ll,edge[0],vv] for edge in lines_edge[ll] if edge[1] == vv) \
	...
	return C4_toRemove, C5_toRemove, C8_toRemove, C6_toFix
...
def getResult(node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, cost_edges, cost_lines, demand_dic, cap_lines, budget, list_f_sluv,\
	...
	# create variables
	f, y, w, z, q, x, Y = createVars(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z, bus_nodes, transit)
	...
	# create constraints
	C4_toRemove, C5_toRemove, C8_toRemove, C6_toFix = createConstrs(m, node_list, od_pairs, ori_nodes, dest_nodes, lines, lines_edge, demand_dic, cap_lines, budget, cost_edges, cost_lines, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z,\
				  f, y, w, z, q, x, Y, bus_nodes, st_dict, ls_dict, svl_dict, z_uvs_dict, y_uvl_dict, alpha)
	
	m.optimize()
	
	return m, m.ObjVal, C4_toRemove, C5_toRemove, C8_toRemove, C6_toFix, f, y, w, z, q, x, Y
...
def newLineLP(m, C4_toRemove, C5_toRemove, C8_toRemove, C6_toFix, cap, budget, od_pairs, node_list, new_lines, lines_edge, ori_nodes, cost_edges, new_line, new_cost_lines, new_cap_lines, new_dict_stlInd_uInd, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z\
	...
	C5_toRemove = m.addConstrs((-x[uu,vv] + gp.quicksum(q[u,v] for (u,v) in [(uu,vv)] if (u,v) in od_pairs) + gp.quicksum(z[s,vv,uu] for s in z_uvs_dict[uu,vv]) + gp.quicksum(y[uu,l,vv]\
														for l in y_uvl_dict[uu,vv]) <= 0 for uu in node_list for vv in node_list), name ='k')
	C6_toFix = m.addConstrs((-new_cap_lines[ll]*Y[ll] + gp.quicksum(f[s, ll, edge[0], edge[1]] for s in ls_dict[ll]) <= 0 \
								for ll in [len(new_lines) - 1] for edge in lines_edge[ll]), name = 'constraint_6')
	...
	return m, C4_toRemove, C5_toRemove, C8_toRemove, C6_toFix, f, y, w, z, q, x, Y, lines_edge, list_f_sluv, list_y_slv, list_w_slv, list_z_stv, list_z\
		...
		for constr in m.getConstrs():
			if constr.ConstrName.split('[')[0] == consrtName_string:
				if consrtName_string == "a" or consrtName_string == "k" or consrtName_string == "p": 
					...
					item = re.findall("\[(.*?)\]", constr.ConstrName)[0]
					var_current[int(item)] = constr.Pi
					print(item) 
					print(constr.Pi)
...
def extract_lines(city, demand_dic, gamma, cap, flex_dist, unit_dist, some_lines):
	...
	# define pair_lineInd_dict, pair_lineInd_dict
